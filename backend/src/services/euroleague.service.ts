...
const BASE_URL = process.env.EUROLEAGUE_API_BASE_URL || 'https://feeds.incrowdsports.com/provider/euroleague-feeds/v2';
...
export class EuroleagueService {
    ...
    async getGame(seasonCode: string, gameCode: string): Promise<ApiResponse<Game>> {
        const response = await axios.get(`${BASE_URL}/competitions/E/seasons/${seasonCode}/games/${gameCode}`);
        return response.data;
    }
